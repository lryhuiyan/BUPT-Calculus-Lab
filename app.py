...
# ==========================================
# 📊 主页面：利用 @st.fragment 实现局部极速刷新
# ==========================================
st.title("🚀 基于DeepSeek V3的微积分绘图工具")

# 定义一个片段，点击片段内的按钮只会局部刷新图像，速度提升 5 倍以上
@st.fragment
def render_visualization(expr, is_3d):
    # 🕹️ 自定义控制面板 (点击这里不会刷新整个页面)
    st.markdown("##### 🎮 视图控制")
    c1, c2, c3, c4 = st.columns([1, 1, 2, 1])
    with c1:
        if st.button("➕", use_container_width=True): 
            st.session_state.zoom_factor *= 0.7
            st.rerun()
    with c2:
        if st.button("➖", use_container_width=True): 
            st.session_state.zoom_factor *= 1.4
            st.rerun()
    with c3:
        if is_3d:
    ...
    if is_3d:
        ...
        if fig:
            ...
            st.plotly_chart(fig, use_container_width=True, config=config, key="v_3d")
    else:
        ...
        if fig:
            fig.update_layout(
                uirevision=True, dragmode='pan', height=550,
                xaxis=dict(range=[-10*z, 10*z]), yaxis=dict(range=[-10*z, 15*z])
            )
            st.plotly_chart(fig, use_container_width=True, config=config, key="v_2d")
